...
std::vector<std::vector<int>> fillRightArrow(
    std::vector<std::vector<int>> matrix, int row, int start_col, int last_col,
    int current_value) {
...
std::vector<std::vector<int>> fillDownArrow(
    std::vector<std::vector<int>> matrix, int col, int start_row, int last_row,
    int current_value) {
...
std::vector<std::vector<int>> fillLeftArrow(
    std::vector<std::vector<int>> matrix, int row, int start_col, int last_col,
    int current_value) {
  for (int j = start_col; j >= last_col; j--) {
    matrix[row][j] = current_value;
...
std::vector<std::vector<int>> fillUptArrow(std::vector<std::vector<int>> matrix,
                                           int col, int start_row, int last_row,
                                           int current_value) {
  for (int i = start_row; i >= last_row; i--) {
    matrix[i][col] = current_value;
    current_value++;
...
std::vector<std::vector<int>> solution(int n) {
  ...
  std::vector<std::vector<int>> answer =
      std::vector<std::vector<int>>(n, std::vector<int>(n_col, default_value));
  int current_value = 1;
  ...
  while (true) {
    answer = fillRightArrow(answer, right_arrow_args["row"],
                            right_arrow_args["start_col"],
                            right_arrow_args["last_col"], current_value);
    current_value =
        answer[right_arrow_args["row"]][right_arrow_args["last_col"]];
    ...
    answer = fillDownArrow(answer, down_arrow_args["col"],
                           down_arrow_args["start_row"],
                           down_arrow_args["last_row"], current_value);
    ...
    answer = fillLeftArrow(answer, left_arrow_args["row"],
                           left_arrow_args["start_col"],
                           left_arrow_args["last_col"], current_value);
    ...
    answer =
        fillUptArrow(answer, up_arrow_args["col"], up_arrow_args["start_row"],
                     up_arrow_args["last_row"], current_value);
