...
std::vector<std::vector<int>> insertSolution(std::vector<std::vector<int>> arr){
    int n_rows = size(arr);
    ...
    if (row_minus_col > 0){
        std::vector<int> additionalCols = std::vector<int>(row_minus_col, default_value);
...
int main(){
    ...
    // Average execution time of insertSolution over 100 iterations: 1.20982e-06 seconds
    // solution is 1.38698 times faster than insertSolution.

    // ref: stackOverFlow(https://stackoverflow.com/questions/13324431/c-vectors-insert-push-back-difference)
