...
std::vector<int> solution(int n, std::vector<int> slicer, std::vector<int> num_list){
    ...
    int c = slicer[2];
    if (n == 1){
        return std::vector(num_list.begin()+0, num_list.begin()+b+1);
    }
    if (n == 2){
        return std::vector(num_list.begin()+a, num_list.end());
    }
    if (n == 3){
        return std::vector(num_list.begin()+a, num_list.begin()+b+1);
    }
    if (n == 4){
