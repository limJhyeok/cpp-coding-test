...
double measureAverageExecutionTime(const std::function<void()>& func, int iterations) {
...
void compareFunctionSpeeds(const std::function<void()>& func1, const std::function<void()>& func2, int iterations) {
    // 각 함수의 평균 실행 시간 측정
    ...
    std::cout << "Average execution time of func1 over " << iterations << " iterations: " 
              << averageTimeFunc1 << " seconds\n";
    std::cout << "Average execution time of func2 over " << iterations << " iterations: " 
              << averageTimeFunc2 << " seconds\n";
